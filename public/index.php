...
/**
 * Haupteinstiegspunkt mit dynamischer Vite-Asset-Ladung und .env-Support.
 *
 * @file index.php
 */
...
{
    // Braces auf neue Zeile (PSR12)
    global $isDev, $viteHost;

    if ((bool) $isDev) {
        return '
            <script type="module" src="' . $viteHost . '/@vite/client"></script>
            <script type="module" src="' . $viteHost . '/src/js/' . $entry . '.js"></script>
            <link rel="stylesheet" href="' . $viteHost . '/src/scss/' . $entry . '.scss">
        ';
    }
    ...
    // Wir nutzen $entry hier dynamisch für das Manifest
    $jsKey  = 'src/js/' . $entry . '.js';
    $cssKey = 'src/scss/' . $entry . '.scss';

    $jsFile  = (string) ($manifest[$jsKey]['file'] ?? '');
...
<body>
    <div class="hero">
        ...
        <p>Umgebung: <strong>
            <?php echo $isDev ? 'Development (Vite Server)' : 'Production (Dist Bundle)'; ?>
        </strong></p>

        <div class="test-area">
            <p>Ein Bild über den Asset-Helper:</p>
            <img src="<?php echo \asset('assets/images/logo.png'); ?>" alt="Sandbox Logo" style="max-width: 100px;">
        </div>
